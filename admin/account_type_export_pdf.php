...
//    private $_password;
    ...
    public function Data() {
        ...
        $output = "";
        $output .= "
                    <h2>Accounts Category Reports</h2>
    ...
    public function ColoredTable($header,$data) {
        ...
        foreach($data as $row) {
            ...
            $this->Cell($w[1], 6, $row['account_type'], 'LR', 0, 'L', $fill);
            $this->Cell($w[2], 6, $row['prefix'], 'LR', 0, 'L', $fill);
            $this->Cell($w[3], 6, $row['min_balance'], 'LR', 0, 'L', $fill);
            $this->Cell($w[4], 6, $row['interest'], 'LR', 0, 'L', $fill);
...
// column titles
$header = array('ID','Account_type','Prefix','Minimum Balance','Interest','Status');
...
// ob_end_clean();
$pdf->Output('loan_category.pdf', 'D');
